...
fn part2(data: &Vec<Vec<u32>>) {
    let mut final_result = 0;
    for row in 0..data.len() {
        for col in 0..data[0].len() {
            if data[row][col] == 9 {
                final_result = final_result + calc_graded_trailheads(data, (row, col));
            }
        }
    }
...
fn calc_graded_trailheads(
    ...
    match data[row][col] {
        ...
        _ => {
            ...
            if let Some(new_row) = row.checked_sub(1) {
                if data[new_row][col] == data[row][col] - 1 {
                    count = count + calc_graded_trailheads(data, (new_row, col));
                }
            }
            //Down
            if row + 1 < data.len() {
                if data[row + 1][col] == data[row][col] - 1 {
                    count = count + calc_graded_trailheads(data, (row + 1, col));
                }
            }
            //Left
            if let Some(new_col) = col.checked_sub(1) {
                if data[row][new_col] == data[row][col] - 1 {
                    count = count + calc_graded_trailheads(data, (row, new_col));
                }
            }
            //Right
            if col + 1 < data[0].len() {
                if data[row][col + 1] == data[row][col] - 1 {
                    count = count + calc_graded_trailheads(data, (row, col + 1));
                }
            }
            count
        }
